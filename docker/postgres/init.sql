...
--
-- DB 는 delivery_db 하나만 쓰고 서비스별로 스키마를 나눈다.
-- DB 자체는 docker-compose-infra.yaml 의 POSTGRES_DB (= 루트 .env 값) 가 만들고,
-- 이 스크립트는 그 DB 안에서 실행된다.
-- Hibernate 는 CREATE SCHEMA 를 내지 않으므로 스키마는 여기서 미리 만들어야 한다.
--
-- 스키마 이름은 {서비스}_schema 다. DB 이름(delivery_db)과 접미사를 다르게 둬야
