...
Spring Boot Spring Cloud Spring Security PostgreSQL Docker RabbitMQ Redis Gemini API
...
| Domain | 기능 |
|---|---|
| 👤 User | 회원가입, 로그인, JWT 인증, 권한 관리, 계정 정지/해제, 개인정보(이름·Slack ID) 암호화 |
| 🌐 Gateway | API Gateway, 인증 및 요청 라우팅 |
| 📍 Hub | 허브 CRUD, 허브 검색, 허브 캐싱 |
| 🚚 Hub Route | 허브 간 이동 경로 관리, 이동 거리 및 시간 관리 |
| 🏢 Company | 업체 등록 및 관리 |
| 📦 Product | 상품 등록 및 관리 |
| 🛒 Order | 주문 생성, 주문 취소, 주문 조회, Idempotency-Key 기반 중복 주문 방지 |
...
- RabbitMQ 기반 Event Driven Architecture 확대
- Saga Pattern 적용
- 배송 담당자 최적 배정 알고리즘
- AI 기반 배송 경로 최적화
- Kubernetes 운영 환경 구축
...
Spring Cloud, Spring Security, Docker, RabbitMQ, Redis, AI API를 활용하여
