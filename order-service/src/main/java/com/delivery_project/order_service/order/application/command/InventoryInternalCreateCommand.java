 ...
 *
 * 수량은 항상 0 으로 시작한다. 호출하는 쪽이 초기 수량을 정하게 두면
 * 입고 이력 없이 재고가 생겨서 수량이 어디서 왔는지 추적할 수 없다.
 */
public record InventoryInternalCreateCommand(
	...
	public static InventoryInternalCreateCommand from(InventoryInternalCreateRequest request) {
		return new InventoryInternalCreateCommand(
				request.productId(), request.hubId(), request.companyId());
	}

	/** 등록 규칙은 외부 API 와 같아야 하므로 기존 Command 로 바꿔 넘긴다 */
	public InventoryCreateCommand toCreateCommand() {
