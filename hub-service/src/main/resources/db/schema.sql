...
--              psql -h <host> -U <user> -d delivery -f schema.sql
--
...
CREATE TABLE IF NOT EXISTS hub_db.p_hubs (
    id            UUID          NOT NULL,
...
CREATE TABLE IF NOT EXISTS hub_db.p_hub_routes (
    id                UUID           NOT NULL,
...
ALTER TABLE hub_db.p_hubs DROP CONSTRAINT IF EXISTS fk_hubs_parent;
ALTER TABLE hub_db.p_hubs ADD CONSTRAINT fk_hubs_parent
    FOREIGN KEY (parent_hub_id) REFERENCES hub_db.p_hubs (id);

ALTER TABLE hub_db.p_hub_routes DROP CONSTRAINT IF EXISTS fk_hub_routes_departure;
ALTER TABLE hub_db.p_hub_routes ADD CONSTRAINT fk_hub_routes_departure
    FOREIGN KEY (departure_hub_id) REFERENCES hub_db.p_hubs (id);

ALTER TABLE hub_db.p_hub_routes DROP CONSTRAINT IF EXISTS fk_hub_routes_arrival;
ALTER TABLE hub_db.p_hub_routes ADD CONSTRAINT fk_hub_routes_arrival
    FOREIGN KEY (arrival_hub_id) REFERENCES hub_db.p_hubs (id);
...
ALTER TABLE hub_db.p_hubs DROP CONSTRAINT IF EXISTS ck_hub_type_parent;
ALTER TABLE hub_db.p_hubs ADD CONSTRAINT ck_hub_type_parent CHECK (
...
ALTER TABLE hub_db.p_hub_routes DROP CONSTRAINT IF EXISTS ck_hub_route_not_self;
ALTER TABLE hub_db.p_hub_routes ADD CONSTRAINT ck_hub_route_not_self
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_hub_name
    ON hub_db.p_hubs (name) WHERE deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_hub_route
    ON hub_db.p_hub_routes (departure_hub_id, arrival_hub_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_hubs_created_at
    ON hub_db.p_hubs (created_at DESC) WHERE deleted_at IS NULL;

-- parentHubId 로 관할 허브를 찾는다 (목록 검색 · 하위 허브 판정).
CREATE INDEX IF NOT EXISTS idx_hubs_parent
    ON hub_db.p_hubs (parent_hub_id) WHERE deleted_at IS NULL;
